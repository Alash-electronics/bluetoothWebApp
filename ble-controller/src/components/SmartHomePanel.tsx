...
export const SmartHomePanel: React.FC<SmartHomePanelProps> = ({
  ...
  const handleDeleteRoom = (roomId: string, e: React.MouseEvent) => {
    ...
    if (room && confirm(`Delete "${room.name}"?`)) {
      roomSettings.deleteRoom(roomId);
      setRooms(roomSettings.getRooms());
    }
  };
            ...
            return (
              <div key={room.id} className="relative">
                ...
                <button
                  ...
                  className="absolute top-3 right-3 w-8 h-8 bg-red-500 hover:bg-red-600 text-white rounded-full flex items-center justify-center transition shadow-md z-10"
                  title="Delete room"
                >
