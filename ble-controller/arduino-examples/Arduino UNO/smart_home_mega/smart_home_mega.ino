/*
 * HM-10 Smart Home Multi-Room Example (Arduino MEGA - 6 Rooms)
 *
 * This sketch demonstrates smart home control with FULL 6-ROOM support.
 * Compatible with the Smart Home panel in the web app.
 *
 * HARDWARE REQUIREMENTS:
 * - Arduino MEGA 2560 (70 digital pins, 16 analog pins)
 * - HM-10 Bluetooth module
 * - 36 relay modules (6 devices × 6 rooms)
 * - 3 sensors (shared across all rooms)
 *
 * MULTI-ROOM SUPPORT:
 * - Supports 6 rooms on Arduino MEGA
 * - App sends room number (1-6) when switching rooms
 * - All commands (L/l, W/w, etc.) apply to current active room
 * - Each room has independent device states and temperature
 *
 * HOW TO CONFIGURE:
 ...
 * 4. Upload to Arduino MEGA and connect from the app
 *
 * Hardware connections:
 * HM-10 TX -> MEGA Pin 10 (RX)
 * HM-10 RX -> MEGA Pin 11 (TX) via voltage divider
 *
 * Room 1: Pins 22-27
 * Room 2: Pins 28-33
 * Room 3: Pins 34-39
...
// Pin arrays for each room (6 rooms for Arduino MEGA)
// Format: {LED, Window, Music, Door, Fan, AC}
const int roomPins[6][6] = {
  // Room 1 - Digital Pins 22-27
  {22, 23, 24, 25, 26, 27},
  // Room 2 - Digital Pins 28-33
  {28, 29, 30, 31, 32, 33},
  // Room 3 - Digital Pins 34-39
  {34, 35, 36, 37, 38, 39},
  // Room 4 - Digital Pins 40-45
  {40, 41, 42, 43, 44, 45},
  // Room 5 - Digital Pins 46-51
  {46, 47, 48, 49, 50, 51},
  // Room 6 - Pins 52, 53, A8-A11
  {52, 53, A8, A9, A10, A11},
};
...
// Device states per room (6 rooms)
...
void setup() {
  ...
  // Setup device pins for ALL 6 rooms
  ...
  Serial.println("HM-10 Smart Home (MEGA - 6 Rooms) Ready");
  Serial.println("36 relays configured (6 devices × 6 rooms)");
  bluetooth.println("Smart Home MEGA Online");
  bluetooth.println("6 Rooms Ready");
}
...
void handleCommand(String cmd) {
  ...
  // Room selection (1-6 for Arduino MEGA)
